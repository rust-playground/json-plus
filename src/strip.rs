...
///
/// Note: This does NOT remove nulls inside arrays unless ALL are null and the [Strip] `Null` | `Empties`
/// options are set due to the potential for re-ordering indexes where each may have a specific
/// meaning.
/// ```rust
...
fn strip_mut_inner(mask: BitFlags<Strip, u8>, value: &mut Value) -> bool {
    match value {
        ...
        Value::Array(a) => {
            // We do NOT remove nulls inside arrays unless ALL are null and null | empties is set
            // due to the potential for re-ordering indexes where each may have a specific meaning.
