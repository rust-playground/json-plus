...
///
/// If the `old` Value was `Value::Null` then the `new` Value is returned BUT with the `null` and
/// `empty` values stripped as they have not been changed compared to the old as there was no old
...
pub fn diff(old: &Value, new: &Value) -> Option<Value> {
    match old {
        Value::Null => crate::strip(Strip::Nulls | Strip::Empties, new.clone()),
        Value::Array(o) if new.is_array() => diff_array(o, new.as_array().unwrap()),
...
fn diff_map(old: &Map<String, Value>, new: &Value) -> Option<Value> {
    ...
    // need to go over old records first, it's the only want to know new data is no longer present.
    for (k, v) in old {
        match new_obj.get(k) {
...
mod tests {
    ...
    #[test]
    fn array() {
        assert_eq!(diff(&vec!["val1"].into(), &Value::Null), Some(Value::Null));
        assert_eq!(
